...
class PromoterModuleDFM(GeneralModule):
    def __init__(self, args, ar_args):
        super().__init__(args)
        ...
        print(f"!!!!!mode is {self.args.mode}")
    ...
    def general_step(self, batch, batch_idx=None):
        ...
        if self.args.mode == 'dirichlet' or self.args.mode == 'riemannian':
            ...
            mask = ~(torch.arange(L, device=self.device) < (1-mask_prob) * L)
            ar_xt = torch.where(mask, 4, seq) # mask token has idx 4
            ar_xt = torch.nn.functional.one_hot(ar_xt, num_classes=5) # AR model
            ar_alphas = mask_prob.expand(B) # AR model
            ...
            ar_xt = torch.nn.functional.one_hot(xt, num_classes=5) # AR model
            ar_alphas = mask_prob.expand(B) # AR model
        ...
        ar_logits = self.ar_model(ar_xt, signal=signal, t=ar_alphas) # AR model
        ...
        if self.stage == "val":
            ...
            if self.args.mode == 'dirichlet':
                ...
                # Absorb escape
                ##############
                seq_pred_refined = self.ar_absorb_escape(seq, signal, logits_pred, seq_pred)
                seq_pred = seq_pred_refined
    ...
    def general_step_refine(self, batch, batch_idx=None):
        self.iter_step += 1
        seq_one_hot = batch[:, :, :4]
        seq = torch.argmax(seq_one_hot, dim=-1)
        signal = batch[:, :, 4:5]
        ...
        if self.stage == "val":
            ...
            if self.args.mode == 'dirichlet':
                ...
                logits_pred = self.distill_inference(seq, signal)
                print(f"shape of logits_pred from distill: {logits_pred.shape}")
                seq_pred = torch.argmax(logits_pred, dim=-1)
                print(f"shape of seq_pred from distill: {seq_pred.shape}")
                print(f"First 10 elements of seq_pred from distill: {seq_pred[0][:10]}")
            else:
                raise NotImplementedError()
            self.lg('seq', [''.join([['A','C','G','T'][num] for num in seq]) for seq in seq_pred])
            seq_pred_one_hot = torch.nn.functional.one_hot(seq_pred, num_classes=self.model.alphabet_size).float()
    ...
    def ar_inference(self, seq, signal):
        ...
        print(f"shape of curr: {curr.shape}") # torch.Size([128, 1024])
        print(f"First 10 elements of curr[0]: {curr[0][:10]}")
        for i, k in enumerate(order):
            t = torch.tensor( i/L ,device=self.device)
            # logits shape: torch.Size([128, 1024, 4])
        ...
        print(f"shape of curr: {curr.shape}")
        print(f"First 10 elements of curr[0]: {curr[0][:10]}")
        return curr

    @torch.no_grad()
    def ar_absorb_escape(self, seq, signal, diffusion_logits_pred, diffusion_seq_pred, absorb_threshold=0.60):
        normalized_logits = torch.nn.functional.softmax(diffusion_logits_pred, -1) # B x L x 4
        max_logits, _ = torch.max(normalized_logits, dim=2) # B x L
        final_res = torch.zeros_like(seq, device=self.device, dtype=torch.long)
        B, L = seq.shape
        print(f">>>>>> Micro Batch <<<<<<<<")
        for i in tqdm(range(B)):
            # print(f"Original sequence: {diffusion_seq_pred[i][:20]}")
            final_res[i] = self.ar_absorb(signal[i],
                                          max_logits[i],
                                          diffusion_seq_pred[i].clone(),
                                          absorb_threshold,
                                          L)
            # print(f"Refined  sequence: {final_res[i][:20]}")
            # print(f"Seq: {seq[i][:10]}")
            # print(f"Ori seq shape: {diffusion_seq_pred[i].shape}")
            # print(f"Ref seq shape: {final_res[i].shape}")
            # print("==========================")
        return final_res
        # Need to deal with the sequence one by one instead of batchwise
        # B, L = seq.shape
        # order = np.arange(L)
        # if self.args.mode =='ardm': np.random.shuffle(order)
        # curr = (torch.ones((B, L), device=self.device) * 4).long()
    ...
    @torch.no_grad()
    def ar_absorb(self, signal, max_logits, pred_seq, absorb_threshold, seq_len):
        i = 0
        full_seq_flag = False
        while i < seq_len:
            if max_logits[i] < absorb_threshold:
                # print(f"absorb at position {i}")
                length, sub_seq, full_seq_flag = self.ar_escape(pred_seq[:i].clone(),
                                                             max_logits,
                                                             signal,
                                                             seq_len)
                # print(f"escape at position {length}")
                if length > i:
                    # print(f"sub seq shape {sub_seq.shape}")
                    # pred_seq = sub_seq + pred_seq[i+length:] if not full_seq_flag else sub_seq
                    pred_seq[:length] = sub_seq
                i = length
            i += 1
            if full_seq_flag:
                break
    ...
    @torch.no_grad()
    def ar_escape(self, prev_seq, max_logits, signal, seq_len):
        curr = (torch.ones((1, seq_len), device=self.device) * 4).long()
        curr[0, :len(prev_seq)] = prev_seq
        length = 0
        full_seq_flag = True
        ori_len = len(prev_seq)
        for j in range(ori_len, seq_len):
            t = torch.tensor( j/seq_len ,device=self.device)
            logits = self.ar_model(torch.nn.functional.one_hot(curr, num_classes=5), signal.unsqueeze(0), t[None].expand(1))
            length = j
            max_cur_token_logits = torch.nn.functional.softmax(logits[:, j], -1).max()
            if max_logits[j] > max_cur_token_logits:
                full_seq_flag = False
                break
            curr[0, j] = torch.distributions.Categorical(probs=torch.nn.functional.softmax(logits[:, j] / self.args.flow_temp, -1)).sample()
        return length, curr[0, :length], full_seq_flag
    ...
    def on_validation_epoch_start(self) -> None:
        ...
        self.sei.load_state_dict(upgrade_state_dict(
            torch.load('/mnt/data/zl6222/neurIPS_rebuttal/data/promoter_design/best.sei.model.pth.tar', map_location='cpu')['state_dict'],
            prefixes=['module.']))
        self.sei.to(self.device)
        self.generator = np.random.default_rng(seed=137)
        logger.info('Finish loading sei model')
